...
### タイマー速報が「だんだん」遅れる件（2026-08-22 発生・原因箇所は未確定）

配信を続けるほど時計が遅れ、**遅れが毎秒0.75秒ずつ増える**。観戦・記録の両画面で同じに出る。
実測では 20秒 → 27秒。時計が実時間の1/4の速さでしか進まない。

**確かなこと**：どこかに行列ができ、送られてくる件数より書き込みが遅くて溜まっていた。
遅れが直線的に増えるのは行列の signature で、通信のゆらぎではこうならない。

**未確定なこと**：**行列が AWS 側にあったのか、端末（iPad）の MQTT 送信待ちにあったのか。**
2026-08-24 に **Wi-Fi の異なる別環境で10分流しても再現しなかった**（Lambda は v3 のまま）。
Lambda も Firestore も両環境で同一なので、**差は回線か端末側にある**可能性が高い。
当初 Firestore の「同一ドキュメント毎秒1回」上限が原因と書いたが、それでは再現しない説明がつかない。
**この推測だけで調査を進めないこと。** 必ず下の数字で切り分ける。

| 見る値 | 大きければ | 対処 |
|--------|-----------|------|
| `uplinkMs` | 端末 → IoT で滞留（回線・TimerLinkアプリ側） | **Lambda では直せない**。セルラーに変える等 |
| `queueMs` | IoT → Lambda で滞留（AWS側） | v5 が捨てて抑える |
| `lagMs` | 表示の遅れの実体（上2つの合計に相当） | 5秒以内なら正常 |

`uplinkMs` を出すには IoT Rule の SQL に `timestamp() AS iotTs` が必要。

**対処は `timekeeperToFirestore_v5.zip`**（v4 は欠陥があったため削除済み）。
古いメッセージを捨てて行列を育てない。パラメータは環境変数で会場でも変えられる。

| 環境変数 | 既定 | 意味 |
|----------|------|------|
| `MAX_LAG_MS` | 5000 | これより古ければ書かずに捨てる |
| `MIN_WRITE_MS` | 800 | 同一デバイスへの書き込み最短間隔 |
| `FORCE_AFTER_MS` | 3000 | この間書けていなければ古くても1件通す（無音を作らない） |

- **`MAX_LAG_MS` を小さくしすぎてはいけない。** v4 では1.5秒にしていたが、回線が遅くて
  常時2〜3秒かかる会場では渋滞していなくても全件が「古い」と判定され、逆効果になる
- **記録画面の「● LIVE」だけを見て判断しないこと。** 2026.08.22-1 より前は
  「最後に受信してからの経過」を出していたため、古い値が毎秒届き続ける症状では
  26秒遅れていても「● LIVE」と表示された。現在は `lagMs` を使って「● N秒遅れ」を出す。
  ただし `lagMs` は Lambda v5 以降でしか入らない
- 切り分けは **TimerLink 実機の表示とアプリを並べて数字を比べる**のが確実
- **会場のWi-Fiで事前に10分の通し確認をする。** 回線が変わると再現しない
