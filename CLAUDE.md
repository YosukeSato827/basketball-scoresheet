...
### iPhone で開けない件（2026-08-18・調査中）

Chrome でタブ6個の状態で molten-scorelink.web.app（運営ページ）を開くと、
`onSnapshot` が最初の1件も返さずエラーも投げないまま無反応。

- **オフライン永続化は原因ではない**。`オフライン保存 無効（自動復旧）` まで進んでも読めなかった。
  **IndexedDB 説は棄却済み**（同じ推測を繰り返さないこと）
- 洋輔さんの報告では**ログイン後は開ける／ログイン前がエラー**。未確認だが重要な手がかり
- **Chrome を立ち上げ直したら復旧した**。ブラウザ側に残る一時的な状態（接続やタブ）が
  詰まっていたと見られる。データやルールの問題ではない
- 再発したら `直接通信テスト` と `認証の初期化` の表示をまず見る。
  当日の応急処置は「ブラウザを完全に終了して開き直す」「タブを減らす」
